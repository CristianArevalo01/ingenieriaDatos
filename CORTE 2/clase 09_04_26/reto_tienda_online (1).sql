...
select AVG(precioProducto) from producto as promedio;
...
insert into cliente(nombreCliente,emailCliente,ciudad) 
values ('Cristian Arevalo','cristian@mail.com','Bogota'),
('Sara Ferro','sara@mail.com','cali'),
('Alisson Nawal','alisson@mail.com','Medellin');

insert into pedidos values
	(1,1,now(),'pendiente',50000),
    (2,2,now(),'enviado',200000);
...
###Tarea 09/04/2026 join con 3 tablas cliente pedido producto
select 
c.nombreCliente as cliente,
p.id_pedido,
p.fecha_pedido,
...
order by p.fecha_pedido desc, c.nombreCliente;

##=== procedimientos almacenados - funciones - vistas(consultas virtuales que no existen))

/* Procedimientos almacenados Stored Procedures 
Son bloques de codigo de sql que tienen un nombre que se almacenan en el sevidor y se ejecutan con invocación o llamandolos CALL
pueden ser de registro o creacion, consulta, modificacion o actualizacion, o de eliminacion
Existen diferentes tipos:
con parametros de entrada in, salida out o ambos(inout)
Sintaxis: 
--Crear procedimiento
DELIMITER//
...
BEGIN
---- delaración de variables locales
DECLARE variable tipo DEFAULT valor;
---- cuerpo del procedimiento
--sentencias sql, control de flujo....
END//
DELIMITER;

-- Invocar procedimiento
CALL nombreProcedimiento(valor_entrada, o @variableSalida, o @variable_entreda_salida);
*/
...
select @pedido_id as id_pedido, @mensaje as mensaje from cliente;
select * from detalle_pedido;
select * from producto;

## Tarea: ejemplo de procedimiento con cursor
...
## validar que el pedido existe y pertenece al cliente indicado, si no debe mostrar mensaje de error
...
## actualizar a restaurar el stock de cada producto de ese pedido (detalle_pedido)
...
## 2. No exitosa el pedido no existe o no pertenece
...
BEGIN
    ...
    -- excepciones para transacciones
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SET p_mensaje = 'Error: Transacción revertida. No se pudo cancelar el pedido.';
    END;

        
## Tarea convertir los dos procedimientos en una vista create view nombreVista (no se pueden hacer los update por que no son consultas)