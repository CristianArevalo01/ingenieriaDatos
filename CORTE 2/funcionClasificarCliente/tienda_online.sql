...
nombre,
precio,
fn_descuento_volumen(25,precio) as TotalConDescuente
from productos;

/* Vistas 
Definición: consulta SQL temporal que se almacena en la base de datos
Sintaxis:
create or replece view nombre_vista as
select...;
usar vista:
select * from nombre_vista;
ver la estructura o definicion
show create view nombre_vista;
eliminar una vista;
drop view if exist nombre_vista;
*/

-- Función previa a la vista (tarea)
DELIMITER //
create function fn_clasificar_cliente(p_id_cliente int)
returns varchar(50)
deterministic 
reads sql data
begin
    ...
    -- calcular el total gastado excluyendo pedidos cancelados
    select coalesce(sum(total), 0)
    into v_monto_total
    from pedidos
    where id_cliente=p_id_cliente
		and estado != 'cancelado';
	-- clasificar segun el monto
    return case
		when v_monto_total >= 2000 then 'VIP'
        when v_monto_total >= 500 then 'FRECUENTE'
        when v_monto_total >= 100 then 'REGULAR'
        when v_monto_total >= 0 then 'NUEVO'
        else 'INACTIVO'
	end;
end//
DELIMITER ;
select * from pedidos;
SELECT fn_clasificar_cliente(1) AS segmento;
...
select nombre as cliente, segmento from v_resumen_clientes where ciudad='MTY' order by monto_total desc;
